    ...
    sock.ev.on('messages.upsert', async (ev) => {
      try {
        ...
        for (const msg of ev.messages) {
          ...
          // typing দেখানো (fail করলেও পরের কোড চলবে)
          try { await sock.sendPresenceUpdate('composing', jid); } catch (e) { console.log('presence err', e.message); }

          const reply = await askAI(jid, text);
          console.log('AI reply:', reply);

          if (reply) {
            const delay = Math.min(4000, 1200 + reply.length * 20);
            await sleep(delay);
        ...
        console.log('upsert handler err', e.message);
      }
    });
  } catch (err) {
...
async function askAI(jid, userText) {
  if (!KEY) { console.log('No API key set'); return 'AI key not set'; }
...
function buildPage() {
  ...
  p.push('button{margin-top:16px;padding:10px 20px;background:#25D366;color:#000;border:none;border-radius:8px;font-size:15px;font-weight:bold}</style>');
  p.push('</head><body>');
  p.push('<h2>WhatsApp Bot Login</h2>');
  p.push('<div id="box">Loading...</div><p id="hint"></p><div id="t"></div>');
  ...
  p.push('else{sec++;box.textContent="Generating QR...";hint.textContent="Please wait";if(sec>4){t.textContent="Taking long? Tap New QR / Reset.";}}}');
  ...
  p.push('load();setInterval(load,3000);');
