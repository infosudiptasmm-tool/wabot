  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (update.connection === 'close') {
        ...
        // logout হলে auth মুছে fresh QR আনতে হবে
    ...
    sock.ev.on('messages.upsert', async (ev) => {
      ...
      for (const msg of ev.messages) {
        ...
        if (reply) {
          const delay = Math.min(5000, 1500 + reply.length * 30);
          await sleep(delay);
...
function buildPage() {
  ...
  p.push('.ok{color:#25D366;font-size:22px}p{color:#aaa}');
  p.push('#t{color:#25D366;font-size:14px;margin-top:10px}');
  ...
  p.push('else{sec++;box.textContent="Generating QR...";hint.textContent="Please wait a few seconds";if(sec>4){t.textContent="Taking long? Tap New QR / Reset below.";}}}');
...
// নতুন QR / Reset: auth মুছে আবার শুরু
